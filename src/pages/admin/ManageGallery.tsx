    ...
    <div className="max-w-7xl mx-auto">
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden max-h-[92dvh] flex flex-col border border-gray-100 my-auto">
            <div className="flex justify-between items-center p-4 sm:p-6 border-b border-slate-100 bg-slate-50/70 shrink-0">
              <h2 className="text-lg sm:text-xl font-bold text-slate-900">Upload New Image</h2>
              <button 
                onClick={() => setShowUploadModal(false)}
                className="text-slate-400 hover:text-slate-600 transition-colors p-2 hover:bg-slate-200/80 rounded-full cursor-pointer active:scale-90"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleUpload} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1 scrollbar-none text-xs sm:text-sm">
                <div>
                  <label className="block font-bold text-slate-700 mb-1.5">Image File</label>
                  <div className="border-2 border-dashed border-slate-200 rounded-2xl p-5 flex flex-col items-center justify-center text-center bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer"
